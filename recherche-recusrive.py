    ...
    def seasonid(self):
        ...
        while True:
            ...
            try:
                ...
                if (self._nom_fichier[scanner] == 's' or self._nom_fichier[scanner] == 'S') and (
                        self._nom_fichier[(scanner + 1)] in str(list(range(0, 10)))) and self._nom_fichier[
                    (scanner - 1)] == '.':
    ...
    def seasonid3(self):
        ...
        for i in range(0, longeur_nom_fichier):
            ...
            if (self._nom_fichier[i] in ['s', 'S']) and (self._nom_fichier[(i + 1)] in str(list(range(0, 10)))) and \
                ...
                print(
                    '{} |detecte| {} {} (.seasonid3)'.format(''.join(self._nom_fichier), series_title, series_episode))

                # rename(self._path_complet, series_pathto)

            if self._nom_fichier[i] in ['1', '2'] and self._nom_fichier[i + 1] in str(list(range(0, 10))) \
                    and self._nom_fichier[i + 2] in str(list(range(0, 10))) and self._nom_fichier[i + 3] in str(
                list(range(0, 10))) \
    ...
    def classify(self):
        ...
        for i, n in enumerate(self._nom_fichier):
            ...
            try:
                ...
                if (self._nom_fichier[i] in ['s', 'S']) and (self._nom_fichier[(i + 1)] in range_1_a_10) and (
                        self._nom_fichier[(i - 1)] in ['.', ' ']):
                    ...
                    series_pathto = ('{}\{}\{}\{} {}{}'.format(dossier_series, series_title_clean, seasons_folder,
                                                               series_title_clean, series_episode, self._extension))
                    ...
                    print('{} |detecte| {} {} (.seasonid3)'.format(''.join(self._nom_fichier), series_title,
                                                                   series_episode))
